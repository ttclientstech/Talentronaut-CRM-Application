...
import {
    ...
    Plus, User, ChevronDown, Save, ExternalLink
} from 'lucide-react';
...
export default function LeadDetailPage() {
        ...
        await fetch(`/api/leads/${params.id}`, {
            ...
            body: JSON.stringify({
                addRemark: {
                    note: remarkNote,
                    method: remarkMethod,
                    lastContactedDate: remarkDate || undefined,
                }
            }),
        });
        setRemarkNote('');
        setRemarkDate('');
        setRemarkMethod('Call');
        await fetchLead();
    ...
    const handleAddMeeting = async (e: React.FormEvent) => {
        ...
        await fetch(`/api/leads/${params.id}`, {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                addMeeting: { title: meetingTitle, date: meetingDate, link: meetingLink }
            }),
        });
        setMeetingTitle('');
        setMeetingDate('');
    ...
    if (!lead) return (
        <div className="flex h-screen flex-col items-center justify-center gap-4 bg-gray-50">
            ...
            <p className="text-sm font-medium text-gray-400">This lead may have been deleted or the link is invalid.</p>
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            {/* Top Bar */}
            <div className="sticky top-0 z-20 bg-white/80 backdrop-blur-xl border-b border-gray-100 px-8 py-4 flex items-center justify-between shadow-sm">
            ...
            <div className="max-w-7xl mx-auto px-8 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                {/* ── LEFT COLUMN: Lead Info + Status + Meetings ── */}
                                    ...
                                    <div key={m._id} className="rounded-2xl border border-gray-50 bg-gray-50/50 p-5">
                                        ...
                                        <div className="mt-3 flex gap-2 flex-wrap">
                                            {['Completed', 'Rescheduled', 'Cancelled'].map(s => (
                                                <button
                                                    key={s}
                                                    onClick={() => handleUpdateMeetingStatus(m._id, s)}
                                                    disabled={m.status === s}
                        ...
                        <form onSubmit={handleAddRemark} className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select
                                        value={remarkMethod}
                                        onChange={e => setRemarkMethod(e.target.value)}
